...
## 4단고음
https://programmers.co.kr/learn/courses/30/lessons/1831?language=cpp
...
5. map을 사용하는 이유는 그냥 배열로 하면 공간복잡도가 너무 증가하기 때문이다. 
6. 거꾸로 간다고 생각하면 쉽다. 
7. map을 check할 때 if(mp.count({pcnt, n})) return mp[{pcnt, n}]; 가 더 효율이 높다. 이걸 안할경우 시간초과가 걸린다. 
...
(m - 1) * 54 괄호처리하는 것을 주의하자. floodfill
...
배열을 이용할 생각. 
```js
let visited = Array.from(new Array(N + 1), () => 0);
let notClear = Array.from(new Array(N + 1), () => 0);
stages.forEach(e =>{
    for(let i = 1; i <= e; i++){
        visited[i]++;
    }
...
k를 중심으로가 아닌 foodTimes의 길이를 중심으로 하면 효율적으로 할 수 있다. 
시간초과가 뜬다면 시간복잡도를 중심으로 반복문의 효율성을 따져보자. map을 사용한다고 해서
효율성이 감소하지는 않는다. 