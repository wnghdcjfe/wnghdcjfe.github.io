...
#### 재귀함수
재귀함수 copy가 재귀적으로 작동하면서 해당 value가 object임을 검사함과 동시에 null이 아님을 검사해서 객체가 만약에 또 있다면 또다시 재귀를 돌리는 형식입니다.
```js
const copy = o =>{ 
    let out = {}; 
    let value, key;  
...
흔히들 UX를 향상시키자고 합니다. UX를 향상시킨다는 것은 홈페이지를 눌렀을 때 빠르게 뜨는 것이 포함이 됩니다. 여러가지가 있지만 이것은 제일 중요한 요소 중 하나입니다. 사용자가 처음 페이지를 켰을 때 필요한 것은 그렇게 많지 않습니다. 지금 사용자가 보고 있는 영역의 이미지만이 필요할 뿐이고 그 아래의 이미지는 필요하지 않습니다. 그렇기 때문에 사용자가 보고 있는 영역의 이미지만 로드를 하고 추후 나중에 게으르게 이미지를 로드하는 것을  LAZY load라고 합니다. 처음 보여주는 썸내일의 이미지를 저화질로 하는 것도 포함합니다. 
...
``` 
![참좋은예제죠?](https://raw.githubusercontent.com/wnghdcjfe/wnghdcjfe.github.io/master/MEVN/3_img_lazy.PNG)
...
이외에도 template를 사용하는 방법이 있지만 권장하지 않습니다. 
 ...
 > 애초에 이미지의 용량을 줄이는 것도 중요합니다. 이미지 용량을 줄이는 사이트로는 https://compressjpeg.com/ 와 https://tinypng.com/ 를 추천합니다. 
...
이렇게 함수형프로그래밍 패러다임부터 시작해서 어떻게 코드를 짜는지 자바스크립트, css에서 살펴보았습니다. 이제는 실전입니다! 이제 예술품사이트를 만들고 실시간 데이터 시각화 프로그램을 만들 것입니다. 이렇게 실제로 웹 어플리케이션을 구현해서 실력을 키운다면 후에 훌륭한 개발자가 될 수 있을 것입니다