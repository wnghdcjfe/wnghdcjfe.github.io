...
흔히들 UX를 향상시키자고 합니다. UX를 향상시킨다는 것은 홈페이지를 눌렀을 때 빠르게 뜨는 것이 포함이 됩니다. 여러가지가 있지만 이것은 제일 중요한 요소 중 하나입니다. 사용자가 처음 페이지를 켰을 때 필요한 것은 그렇게 많지 않습니다. 지금 사용자가 보고 있는 영역의 이미지만이 필요할 뿐이고 그 아래의 이미지는 필요하지 않습니다. 그렇기 때문에 사용자가 보고 있는 영역의 이미지만 로드를 하고 추후 나중에1 게으르게 이미지를 로드하는 것을  LAZY load라고 합니다. 처음 보여주는 썸내일의 이미지를 저화질로 하는 것도 포함합니다. 
