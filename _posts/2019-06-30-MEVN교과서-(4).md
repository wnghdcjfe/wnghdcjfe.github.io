...
## 3. 코드최적화와 패러다임
코드를 짤 때 그저 로직만을 짜면 절대 안됩니다. 그것은 죄악이며 해야할 짓이 못됩니다. 코드를 짤 때 우리는 확장성, 유연성, 성능 등을 모두 고려해야 합니다. 이렇게 조금씩 그리고 세심하게 짠 코드가 나중에 봤을 때 무한한 행복을 가져올 것입니다. 

### 3.1 함수형프로그래밍
그렇다면 어떻게 짜야 할까요? 한 프로젝트당 코드 패러다임을 정해놓고 짜는 것이 중요합니다.패러다임이란 어떻게 짤 것인지를 규정한 것을 말합니다. 예를 들어서 객체지향형으로 짜자, MVC패턴으로 짜자, 함수형프로그래밍으로 짜자 등을 일컫습니다. 자세히 들어가면 margin: 0 px는 margin : 0 으로 놓자 등 아주 세세한 코드 스타일까지 정하는 것을 일컫습니다. 
...
함수형 프로그래밍을 한다면 FX.js를 사용하는 것이 좋습니다. 우리나라에서 함수형 프로그래밍을 제일 잘하는 사람을 뽑으라면 유인동개발자님을 뽑을 정도로 훌륭한 분이며 lodash, ramda의 장점을 섞어 만든 partial.js의 다음 버전 라이브러리입니다. 
사용하기도 편리하고 성능도 좋습니다. 앞으로 이어나갈 모든 코드들에는 fx.js가 포함될 예정입니다. 
...
5. 배열순회의 경우 for-in 루프를 쓰지 않습니다. (객체 속성순회일경우에는 씁니다.) 
 - 인스턴스, 프로토타입 체인까지 검색하므로 느립니다.
6. 정규표현식은 간단하게 씁니다. 예를 들어 ble|blue 대신 blu?e를 씁니다.

### 3.2.1 함수선택, parsInt와 ~~의 성능비교
### 3.2.2 함수선택, deepCopy 성능비교
### 3.2.3 함수선택, includes, indexOf 성능비교
### 3.2.4 객체선언에 따른 성능비교
### 3.2.5 서버요청최적화 - minify와 HTTP2
### 3.2.6 서버요청최적화 - 메모이제이션 
### 3.2.7 로직최적화 : 피벗알고리즘, 시간복잡도 이미지리시트를 찾는 로직
### 3.2.8 로직최적화 : 서버요청최소화 
### 3.2.9 페이지로드속도최적화 : LAZY load
### 3.2.10 페이지로드속도최적화 :  이미지크기 축소  
